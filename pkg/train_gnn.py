...
class ZIPLoss(nn.Module):
    def __init__(
        ...
        self.eps = eps
        # parameters
        self.total_count = nn.parameter.Parameter(torch.tensor(1.0), requires_grad=True)
    ...
    def ZIP_nll(self, lamda, count, inflat_prob):
        ...
        inflat_prob = torch.sigmoid(inflat_prob)

        zero_nll = -torch.log(
            inflat_prob + (1 - inflat_prob) * torch.exp(-lamda) + self.eps
        )
        non_zero_nll = self.loss_func(lamda, count) - torch.log(1 - inflat_prob + self.eps)
    ...
    def forward(self, y_true, y_pred):
        ...
        target_lambda = self.total_count * torch.exp(y_pred_matrix + y_pred_target)
        ...
        target_nll = self.ZIP_nll(target_lambda, y_true_target, self.target_inflat_prob)
        matrix_nll = self.ZIP_nll(matrix_lambda, y_true_matrix, self.matrix_inflat_prob)
...
model.load_state_dict(torch.load("/data02/gtguo/model.pth"))
...
print(criterion.total_count, criterion.matrix_inflat_prob, criterion.target_inflat_prob)

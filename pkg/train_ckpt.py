...
if __name__ == "__main__":
    ...
    ckpts = glob(os.path.join(args.save_path, args.name, "state_dict_*.pt"))
    if ckpts:
        ckpts = sorted(ckpts, key=lambda x: int(re.search(r"\d+", x).group()))
        ckpt = ckpts[-1]
    ...
    def save_all_paramters(
        model, criterion, optimizer, lr_scheduler=None, fpath="./state_dict.pt"
    ):
        state_dict = {}
    ...
    # * here the epoch ranges from 1 to args.epochs
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % args.save_interval == 0:
            save_all_paramters(
                ...
                fpath=os.path.join(args.save_path, args.name, f"state_dict_{epoch}.pt"),
            )
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(del_val_loader):
                ...
                if args.loss_sigma_correction:
                    ...
                    loss, tgt_nll, mat_nll = criterion(
                        output,
                        torch.cat(
                            (
                                data["readout"][args.target_name]["target"].view(
    ...
    save_all_paramters(
        ...
        optimizer,
        lr_scheduler=scheduler,
        fpath=os.path.join(args.save_path, args.name, "state_dict.pt"),
    )

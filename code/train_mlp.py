...
class ZIPLoss(nn.Module):
    ...
    def forward(self, y_true, y_pred):
        ...
        y_true_target, y_true_matrix = y_true[:, :self.target_size], y_true[:, self.target_size:]
...
# %% recursively move all components of dict to device
def move_to_device(data, device):
    if isinstance(data, dict):
        for key in data:
            data[key] = move_to_device(data[key], device)
        return data
    else:
...
# %% load data
batch_size = 4096
dataset = test_building_block_dataset.BuildingBlockFingerprintDataset()
...
model = fingerprint_mlp.FingerprintMLP().to(device)
...
for e in range(epoch):
    ...
    for data in tqdm(train_loader):
        ...
        y_real = data["target"]["target"]

        optimizer.zero_grad()
        out = model(**data["input"])
        loss = criterion(y_real, out)
        loss.backward()
        optimizer.step()

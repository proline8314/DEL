...
def smiles_to_mol_graph(
    smiles: str, return_mol_instance: bool = False, keep_all_hydrogen: bool = True
) -> nx.Graph | tuple[nx.Graph, Chem.Mol]:
    """
...
def reduce_graph_to_core(mol_graph: nx.Graph) -> nx.Graph:
    ...
    while True:
        ...
        peripheral_atom_indices = [
            atom_idx
            for atom_idx in core_graph.nodes
            if len(list(core_graph.neighbors(atom_idx))) == 1
        ]
        ...
        elif len(peripheral_atom_indices) <= 2 and len(peripheral_atom_indices) == len(
            core_graph.nodes
        ):
            return core_graph
        else:
...
def expand_core_to_skeleton(
    bb_mol_graph: nx.Graph, core_of_bb_mol_graph: nx.Graph, template_mol_graph: nx.Graph
):
    def add_atom(
        mol_graph: nx.Graph,
        new_atom_idx: int,
    ...
    def r_branch_from_single_atom(
        ...
        for neighbor in bb_mol_graph.neighbors(atom_idx):
            ...
            branch_attempt_graph = add_atom(
                branch_attempt_graph,
                neighbor,
                atom_idx,
                bb_mol_graph.nodes[neighbor]["atom_symbol"],
                bb_mol_graph.edges[neighbor, atom_idx]["bond_type"],
            )
            # check atom symbols only
            gm = nx.algorithms.isomorphism.GraphMatcher(
                template_mol_graph,
                branch_attempt_graph,
                node_match=lambda n1, n2: n1["atom_symbol"] == n2["atom_symbol"],
    ...
    public_occupied_atom_indeces = [atom_idx for atom_idx in core_of_bb_mol_graph.nodes]
...
def get_bb_skeleton(bb_mol_graph: nx.Graph, template_mol_graph: nx.Graph):
    ...
    skeleton_of_bb_mol_graph = expand_core_to_skeleton(
        bb_mol_graph=bb_mol_graph,
        core_of_bb_mol_graph=core_of_bb_mol_graph,
        template_mol_graph=template_mol_graph,
    )
...
def match_motif_to_mol(mol: nx.Graph, bb_motif_dict: dict[str, nx.Graph]) -> nx.Graph:
    ...
    def r_find_nearest_motif_assigned_neighbor(
        mol: nx.Graph, node_idx: int, visited: list[int]
    ):
        ...
        if len(passed_motif_list) > 1:
            ...
            warn(
                f"Node {node_idx} has more than one motif-assigned neighbor, only the smallest one {motif_to_be_assigned} is considered"
            )
    ...
    for motif_name, motif in bb_motif_dict.items():
        ...
        gm = GraphMatcher(mol_copy, motif, node_match=node_match, edge_match=edge_match)
        if not gm.subgraph_is_isomorphic():
            is_flexible_matching = False
            continue
    ...
    for node_idx in list(mol_copy.nodes):
        if mol_copy.nodes[node_idx]["motif"] is not None:
            continue
        mol_copy, _ = r_find_nearest_motif_assigned_neighbor(mol_copy, node_idx, [])
    return mol_copy
...
def assign_genetal_linker(mol: nx.Graph):
    ...
    for idx in cmol.nodes:
        ...
        is_start = (
            cmol.nodes[idx]["atom_symbol"] == "C"
            and len(neighbor) == 1
            and cmol.nodes[neighbor[0]]["atom_symbol"] == "N"
        )
        if is_start:
...
def assign_mol(
    mol: nx.Graph, assigned_template_mol: nx.Graph, return_idx: bool = False
):
    cmol = copy.deepcopy(mol)
    for idx in cmol.nodes:
        cmol.nodes[idx]["is_start"] = 0
...
def assign_motif(
    linker_motif: str = DNA_LINKER_MOTIF,
    general_linker: str = GENERAL_LINKER,
    node_attribute: str = "topo_dist",
):
    general_linker_mol = smiles_to_mol_graph(general_linker, keep_all_hydrogen=False)
...
def assign_topo_dist_to_DNA(
    mol: nx.Graph, linker_mol: nx.Graph, node_attribute: str = "topo_dist"
):
...
if __name__ == "__main__":
    ...
    txn = env.begin()
    feat_env = lmdb.open(
        os.path.join(dataset_path, dataset_name, "features.lmdb"),
        subdir=False,
        readonly=False,
        lock=False,
        readahead=False,
    ...
    for key, value in txn.cursor():
        ...
        bb_idxs = [data_dict["BB1_id"], data_dict["BB2_id"], data_dict["BB3_id"]]
        bb_motif_dict = {}
